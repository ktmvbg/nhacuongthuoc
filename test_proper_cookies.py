...
for cookie in raw_cookies:
    ...
    # Map sameSite properly
    same_site = cookie.get("sameSite")
    if same_site:
        same_site_lower = same_site.lower()
        if same_site_lower == "no_restriction":
            pw_cookie["sameSite"] = "None"
        elif same_site_lower in ["lax", "none", "strict"]:
            pw_cookie["sameSite"] = same_site_lower.capitalize()
            
    if expires is not None:
        pw_cookie["expires"] = float(expires)
    playwright_cookies.append(pw_cookie)

print(f"Loaded and mapped {len(playwright_cookies)} cookies.")
...
with sync_playwright() as p:
    ...
    # Save a screenshot
    screenshot_path = r"C:\Users\PC\Desktop\nhacuongthuoc\oauth_proper_cookies_result.png"
